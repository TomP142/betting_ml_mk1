...
import { Terminal, Database, Play, AlertCircle, ChevronsUp, Trophy, Activity, Wifi } from 'lucide-react'
    ...
    const handleRunAnalysis = async () => {
        ...
        try {
            ...
            if (data.results) {
                ...
                addLog(`Analysis Complete. ${data.results.length} predictions received.`)
            }
        } catch (e) {
            ...
            <header className="max-w-7xl mx-auto flex justify-between items-center mb-12">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-3xl font-bold tracking-tighter text-white">
                        BETTING<span className="text-neon-green">.AI</span> <span className="text-xs opacity-50 font-mono border border-white/20 px-2 py-0.5 rounded">MK1</span>
                    </h1>
                </div>
                <div className="flex items-center gap-4 text-sm font-mono text-gray-400">
                    <div className="flex items-center gap-2">
                        <Wifi size={14} className={isConnected ? "text-neon-green" : "text-red-500"} />
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6 lg:col-span-1">
                    ...
                    <div className="glass p-6 rounded-2xl relative overflow-hidden group">
                        ...
                        <div className="mb-4">
                            <label className="block text-xs font-mono text-gray-500 mb-2">TARGET DATE</label>
                            <input
                                type="date"
                                value={selectedDate}
                    ...
                    <div className="glass p-4 rounded-2xl h-[500px] flex flex-col font-mono text-xs relative overflow-hidden">
                        ...
                        <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-1 scrollbar-hide text-green-400/80">
                            ...
                            {predictions.slice(0, 6).map((pred, i) => (
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="text-xl font-bold text-white">{pred.PLAYER_NAME}</h3>
                                            <div className="text-xs text-gray-400 flex items-center gap-2 mt-1">
                                                <span className="bg-white/10 px-1.5 py-0.5 rounded">{pred.MATCHUP}</span>
                                                <span>{pred.GAME_DATE}</span>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-xs text-gray-400">Total Proj.</div>
                                            <div className="text-2xl font-black text-neon-green tracking-tighter">
                                                {(pred.PRED_PTS + pred.PRED_REB + pred.PRED_AST).toFixed(1)}
                                            </div>
                                            <div className="text-[10px] text-gray-500">PRA</div>
                                        </div>
                                    </div>

                                    {/* Stats Grid */}
                                    <div className="grid grid-cols-3 gap-2 mt-4 bg-black/20 p-3 rounded-xl border border-white/5">
                                        <div className="text-center">
                                            <div className="text-[10px] text-gray-400 mb-1">PTS</div>
                                            <div className="font-bold text-lg">{pred.PRED_PTS.toFixed(1)}</div>
                                        </div>
                                        <div className="text-center border-l border-white/5">
                                            <div className="text-[10px] text-gray-400 mb-1">REB</div>
                                            <div className="font-bold text-lg">{pred.PRED_REB.toFixed(1)}</div>
                                        </div>
                                        <div className="text-center border-l border-white/5">
                                            <div className="text-[10px] text-gray-400 mb-1">AST</div>
                                            <div className="font-bold text-lg">{pred.PRED_AST.toFixed(1)}</div>
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
